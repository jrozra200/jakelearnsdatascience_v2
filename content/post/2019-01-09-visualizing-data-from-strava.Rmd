...
I hurt myself early in 2018 and that's all I could do for a while), ect...
...
my_act <- get_activity_list(stoken, id = '5954092')
```
...
I also want to convert a few fields to units that make more sense for me (miles, 
feet, hours instead of meters and seconds).

```{r feature_creation, message = FALSE, warning = FALSE, error = FALSE}
library(dplyr)
library(lubridate)
...
```{r overall_cumulative, warning = FALSE, message = FALSE, out.width = "100%"}
library(ggplot2)
library(ggridges)
...
```{r by_activity, warning = FALSE, message = FALSE, out.width = "100%"}
...
```{r ridgeplot, warning = FALSE, message = FALSE, out.width = "100%"}
